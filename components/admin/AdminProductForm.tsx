  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const data = {
        ...
        isBestSeller,
        isNew,
        image: imageUrl.trim() || "/brand/packaging.png",
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        >
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            <div className="h-40 w-40 shrink-0 overflow-hidden rounded-2xl border-2 border-dashed border-gold/30 bg-cream">
              {imageUrl ? (
                <Image 
                  src={imageUrl} 
                  alt="Preview" 
            ...
            <div className="flex-1">
              <label className="mb-2 block text-sm font-medium text-navy">
                لصق رابط الصورة هنا (من الإنترنت)
              </label>
              <input
                type="url"
                value={imageUrl}
              ...
              <p className="mt-2 text-xs text-navy/50">
                يمكنك نسخ رابط أي صورة من الإنترنت واستخدامها
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="flex flex-wrap gap-4">
          <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} className="group relative flex items-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-navy to-navy-light px-8 py-3.5 text-sm font-medium text-gold shadow-lg shadow-navy/30 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60">
